...
function isConnectionString(options: AzureConnectionOptions): options is AzureBlobServiceConnectionString {
    return (<AzureBlobServiceConnectionString>options).connectionString !== undefined;
}
function isAccountAndKey(options: AzureConnectionOptions): options is AzureBlobServiceAccountAndKey {
 ...
 *               `─────────'      `─────────'         
 */
function createAzureUpload(options: IAzureOptions): (assets: { [assetName: string]: IAsset }) => Observable<string> {
    ...
    if (options.connection && isConnectionString(options.connection)) {
        ...
        blobService = createBlobService(options.connection.storageAccount, options.connection.storageAccessKey, options.connection.host);
    } else {
     ...
     */
    const listBlobsSegmentedWithPrefix: (container: string, prefix: string, currentToken: common.ContinuationToken, options: BlobService.ListBlobsSegmentedRequestOptions) => Observable<BlobService.ListBlobsResult> =
        Observable.bindNodeCallback(
            bind(blobService.listBlobsSegmentedWithPrefix, blobService),
            (result, response) => result
        );
    const createContainerIfNotExists: (container: string, options: BlobService.CreateContainerOptions) => Observable<BlobService.ContainerResult> =
        Observable.bindNodeCallback(
            bind(blobService.createContainerIfNotExists, blobService),
            (result, response) => result
        );
    const createBlockBlobFromText: (container: string, blob: string, text: string | Buffer, options: BlobService.CreateBlobRequestOptions) => Observable<BlobService.BlobResult> =
        Observable.bindNodeCallback(
    ...
    function listAllBlobsSegmentedWithPrefix(
        ...
        return listBlobsSegmentedWithPrefix(container, prefix, currentToken, options)
            .mergeMap(result =>
                Observable.concat(
                    result.entries,
    ...
    return (assets: { [assetName: string]: IAsset }) => Observable.create((observer: Observer<string>) => {
        const assetNames = Object.keys(assets);
        const emittedAssetNames = Object.keys(pickBy(assets, asset => asset.emitted));
        observer.next(`upload: creating Azure blob container '${options.containerName}'`);
        createContainerIfNotExists(options.containerName, { publicAccessLevel: 'blob' })
            .mergeMap(() => {
                observer.next(`upload: listing blobs under '${options.containerName}/${options.prefix}'`);
            ...
            .mergeMap(azureBlobNames => {
                ...
                return Observable.from(emittedAssetNames)
                    .mergeMap((assetName, index) => {
                        observer.next(`upload: creating blob (${index + 1}/${emittedAssetNames.length}) '${options.containerName}/${options.prefix}/${assetName}'`);
                        return createBlockBlobFromText(
                            ...
                            observer.next(`upload: failed '${options.containerName}/${options.prefix}/${assetName}'; trying again`);
                            throw error;
                        }).retry(5);
                    }, null, 50)
                    .count()
                    ...
                    .mergeMap(() => {
                        ...
                        return Observable.from(extraneousBlobs).mergeMap(blobName => {
                            observer.next(`upload: pruning '${options.containerName}/${options.prefix}/${blobName}'`);
                            return deleteBlob(
                                options.containerName,
                                `${options.prefix}/${blobName}`,
                                {}
                            ).catch(error => {
                                observer.next(`upload: delete failed '${options.containerName}/${options.prefix}/${blobName}'; trying again`);
                                throw error;
                            }).retry(5);
                        }, null, 100);
                    })
                ...
                next() { observer.next('upload: done'); },
...
export default class AzurePlugin implements Plugin {
    private upload: (assets: { [assetName: string]: IAsset }) => Observable<string>;
